...
Proxy 可以代理整个对象进行读取、设置、删除属性。通过依赖收集（Dependency Collection）和触发更新（Trigger Update）来实现数据的自动同步（观察者模式）：当一个响应式数据被读取时，Vue 3 会收集当前的依赖（即正在执行的副作用函数）；当响应式数据被修改时，Vue 3 会触发所有收集到的依赖，从而更新视图。

##### 关于 Vapor Mode

将代码编译为直接操作 DOM 的指令，省去 VNode 中间层。并非技术的回退，而是编译器选择最佳的手动操作指令，不依赖开发者的经验，所以也节省了中间层的性能消耗。
...
默认情况下，Proxy 只会直接代理它所包裹的对象的顶层属性。当对象中的属性本身是对象时，Proxy 并不会自动代理这些嵌套对象。因此，想要代理嵌套对象时，需要在访问嵌套对象时动态地为其创建新的代理。
...
const proxyObj = createDeepProxy(nestedObj);

// 设置嵌套对象的属性，代理会捕获到这个变化
proxyObj.a.b.c = 10; // 输出: Setting property "c" to value: 10
proxyObj.a.b = { d: 20 }; // 输出: Setting property "b" to value: { d: 20 }
```
...
- 生态：Vue 提供“官方一体化方案”，React 更强调“社区驱动的自由组合”
- 编译：Vue 优化的是“编译层的渲染效率”，React 优化的是“运行时的调度效率”。

**总结**：React 更强调纯函数与数据不可变性，通过 Fiber 和 Hooks 提升灵活性和性能。
Vue 则强调响应式和工程化体验，通过 Proxy 实现精细依赖追踪、通过编译优化提升性能。
两者最终都趋向于逻辑复用（Hooks / Composition API）、编译优化和跨端渲染，但 Vue 更偏“框架一体化”，React 更偏“生态开放化”。
