...
- **同级比较，不跨层级**：节点层级不同，直接销毁旧节点及其子树，重新创建新节点。
- **同类型节点，复用；不同类型，销毁重建**
- **列表节点通过 key 进行对比**：React 会根据 key 来判断节点是否“是同一个”，来决定进行增删、移动还是复用操作
...
**reconciliation 阶段**：从 vdom 转换成 fiber，并且对需要 dom 操作的节点打上 effectTag 的标记，通过 requestIdleCallback 或 Scheduler 分片执行
...
| 特性       | 普通 VNode（React15/Vue） | React Fiber 节点                 |
| ---------- | ------------------------- | -------------------------------- |
| 数据结构   | 树结构（对象嵌套）        | 链表结构（child/sibling/return） |
| 描述内容   | UI（标签、属性、子节点）  | UI + 状态 + 优先级 + 更新标记    |
| 遍历方式   | 递归遍历                  | 循环遍历，可中断/恢复            |
| 生命周期   | 每次更新新建节点          | 节点可复用，保存上次渲染结果     |
| 调度能力   | 无调度能力                | 支持优先级、并发、异步调度       |
| 双缓存机制 | 无                        | 有（current/workInProgress）     |
...
1. 减小 diff 算法计算量，提升渲染性能
2. 避免列表项有状态的组件状态混乱

## React 事务机制

React15 之前用的方案，一次更新是一笔“事务”（Transaction），React 会从根节点递归更新虚拟 DOM → 计算 diff → 同步更新真实 DOM

## React 并发机制

React 18 引入的新模式，允许 React 在渲染过程中根据任务的优先级进行调度和中断，从而确保高优先级的更新能够及时渲染。在 React 16 中，虽然 Fiber 内部具备了“可中断”的能力，但对开发者暴露的渲染模式依然是“同步模式（legacy mode）”。
...
| React 更新类型                          | 默认优先级           |
| --------------------------------------- | -------------------- |
| 同步更新（legacy mode setState）        | ImmediatePriority    |
| 点击事件、输入事件（onClick、onChange） | UserBlockingPriority |
| 异步请求数据后 setState                 | NormalPriority       |
| 离屏组件渲染                            | LowPriority          |
...
- 逻辑层：类组件 this 模糊性，函数式组件写法简单，无生命周期函数，类组件划分了明确的生命周期
- 工程层：逻辑复用更容易
- 性能层：类组件的生命周期模型是同步阻塞的，限制了 React 18 引入的 Concurrent Mode（并发渲染）
...
官方要求每次渲染都以相同顺序调用 Hooks。顺序变化会导致状态赋值错乱。因为 React 依赖 Hooks 调用顺序的稳定性来关联状态与内存索引 ‌。
React 内部在执行函数组件时，会维护一个“Hook 调用栈”（本质上是一个数组）。每次组件重新渲染时，React 会根据调用顺序，把 hooks[0] 赋给第一个 useState，hooks[1] 赋给第二个。顺序一旦变了，整个状态系统就乱了。

### React 闭包

React 中的闭包问题主要出现在函数组件和 Hooks 中，是由于 JavaScript 闭包特性与 React 重新渲染机制的结合导致的。解决方式包括：使用函数式 setState 更新、正确设置 useEffect 依赖数组、使用 useRef 保存最新值的引用
...
- 所有使用 useContext 的组件都会在 Provider 值变化时全部重新渲染；
- 不方便做复杂逻辑（例如异步请求、副作用、模块化拆分）；
- 调试困难，数据流跟踪不直观。
...
- 状态集中、可预测；
- 社区生态完善（redux-saga、redux-thunk）；
- 适合大型应用的模块化管理；
- 性能优化成熟（useSelector 精准订阅）。
